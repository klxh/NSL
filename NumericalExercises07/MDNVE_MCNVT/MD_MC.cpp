...
// Evaluate tail corrections
    tail_u = 8*pi*rho/3 * (1./3 * pow(rcut, -9) - pow(rcut, -3)) ; // Potential energy
    tail_p = 32*pi*rho*rho/3 * (1./3 * pow(rcut, -9) - 1./2 * pow(rcut, -3)) ; // Pressure 

//Print initial values for measured properties
  cout << "Initial potential energy = " << walker[iv]/(double)npart + tail_u << endl;
  ...
  cout << "Initial pressure         = " << walker[iw] + tail_p << endl;
...
// distance i-j in pbc
      ...
      for(int k = 0; k < nbins; k++){
        if(k*bin_size < dr && dr <= (k+1)*bin_size) { walker[n_props+k] += 2 ;}
      }
    }          
  }
  ...
  walker[iw] = rho * (2.0 / 3.0) * kin/(double)npart + 16/vol * w; // Pressure

  // Normalization of g(r)
  for (int k = 0; k < nbins; k++){
    double dV = 4*pi/3 * ( pow((k+1)*bin_size, 3) - pow(k*bin_size, 3) );
    walker[n_props+k] /= (rho * npart * dV);
  }
...
void Accumulate(void) //Update block averages (added g(r))
{
...
void Averages(int iblk) //Print results for current block
{
   ...
   ofstream Epot, Ekin, Etot, Temp, Pres, gdir, final_gdir;
   const int wd=18;
    ...
    final_gdir.open("output_final_gdir.dat",ios::app);
    
    stima_pot = blk_av[iv]/blk_norm/(double)npart + tail_u; //Potential energy
    ...
    stima_pres = blk_av[iw]/blk_norm + tail_p; //Pressure
    ...
    // g(r)
    for(int k = 0; k < nbins; k++){
        stima_gdir = blk_av[n_props+k] / blk_norm;
        glob_av[n_props+k] += stima_gdir;
        glob_av2[n_props+k] += stima_gdir*stima_gdir;
        err_gdir=Error(glob_av[n_props+k],glob_av2[n_props+k],iblk);

        gdir << setw(wd) << iblk << setw(wd) << (k+.5)*bin_size << setw(wd) << stima_gdir
             << setw(wd) << glob_av[n_props+k]/(double)iblk << setw(wd) << err_gdir << endl;
        ...
        if(iblk==nblk-1){
            ...
            final_gdir << setw(wd) << iblk << setw(wd) << setprecision(8) << (k+.5)*bin_size 
                       << setw(wd) << setprecision(8) << stima_gdir
                       << setw(wd) << setprecision(8) << glob_av[n_props+k]/(double)iblk 
                       << setw(wd) << setprecision(8) << err_gdir << endl;
        }
    }
...
//Kinetic energy
    Ekin << setw(wd) << iblk <<  setw(wd) << stima_kin 
         << setw(wd) << glob_av[ik]/(double)iblk << setw(wd) << err_kin << endl;

//Total energy
    Etot << setw(wd) << iblk <<  setw(wd) << stima_etot 
         << setw(wd) << glob_av[ie]/(double)iblk << setw(wd) << err_etot << endl;

//Temperature
    Temp << setw(wd) << iblk <<  setw(wd) << stima_temp 
         << setw(wd) << glob_av[it]/(double)iblk << setw(wd) << err_temp << endl;
