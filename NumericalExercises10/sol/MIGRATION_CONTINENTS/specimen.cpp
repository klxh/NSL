...
void specimen::init(vertices cities, Random& rnd){
...
int specimen::nucleotide(int spot){
    if(spot < 0 || spot >= m_spec.size()){
        std::cerr << 
        "specimen::nucleotide ERROR MESSAGE\n" << 
        "Trying to access element out of boundaries.\nEXITING PROGRAM" << std::endl;
        std::exit(EXIT_FAILURE); // Exit the program 
    }
    return m_spec[spot];
}
...
void specimen::inversion(double p_i, Random& rnd){
    ...
    if(r <= p_i){
        ...
        do { P1 = rnd.Integer(m_spec.size()); }
        while (P1 == 0); // Position 0 is always left unchanged
        do { P2 = rnd.Integer(m_spec.size()); }
        while (P2 == 0 || P2 == P1);
...
void specimen::translation(double p_t, Random& rnd){
    ...
    if(r <= p_t){
        ...
        P1 = 1 + rnd.Integer(m_spec.size() - 1);
        do { P2 = 1 + rnd.Integer(m_spec.size() - 1); }
        while (abs(P2-P1) == m_spec.size() - 2 || P2 == P1);

        if(P2 <= P1) { int a = P2; P2 = P1; P1 = a; } // So that P1 is always the smallest

        do { T = 1 + rnd.Integer(m_spec.size() - 1); }
        while (T >= P1 && T <= P2);
...
void specimen::switching(double p_s, Random& rnd){
