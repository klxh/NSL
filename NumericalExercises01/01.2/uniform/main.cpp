...
int main (int argc, char *argv[]){
   ...
   if (input.is_open()){
	...
	myfile1.open("./data/uniformN1.dat");
	...
	myfile2.open("./data/uniformN2.dat");
	...
	myfile3.open("./data/uniformN10.dat");

	for(int i = 0; i < samples; i++){
        double r = 0;
		for(int j = 0; j < 10; j++) { r += rnd.Rannyu()/10; }
        myfile3 << r << endl;
	}
	...
	myfile4.open("./data/uniformN100.dat");

	for(int i = 0; i < samples; i++){
        double r = 0;
		for(int j = 0; j < 100; j++) { r += rnd.Rannyu()/100; }
        myfile4 << r << endl;
	}
