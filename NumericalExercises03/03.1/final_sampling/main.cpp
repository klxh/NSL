...
int main (int argc, char *argv[]){
   ...
   if (input.is_open()){
	...
	// Option pricing sampling directly from final distribution
	...
	for(int i = 0; i < N; i++){
		for(int j = 0; j < L; j++){
            // Sampling final prices
			double S = S_0 * exp((r - .5 * sigma * sigma) * T + sigma * sqrt(T) * rnd.Gauss(0,1));
	...
	ofstream myfile1;
	myfile1.open("./data/call_final.dat");
	...
	ofstream myfile2;
	myfile2.open("./data/put_final.dat");
