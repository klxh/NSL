...
void vertices::init(int Nv, const std::string& geometry) {
    m_index.clear();
    m_x.clear();
    m_y.clear();
    ...
    if(geometry=="CIRCLE"){
        ...
        for(int i=0; i<Nv; i++){
            do{
                x = 2*((double)random()/RAND_MAX-.5);
                y = 2*((double)random()/RAND_MAX-.5);
                dr = sqrt(x*x + y*y);
            } while(dr > 1);
            m_x.push_back(x/dr);
            m_y.push_back(y/dr);
        ...
        for(int i=0; i<Nv; i++){
            m_x.push_back((double)random()/RAND_MAX);
            m_y.push_back((double)random()/RAND_MAX);
        }
    }
    else {
...
int vertices::get_vertex(int elem) {
    if (elem < 0 || m_index.size() <= (size_t)elem) {
        std::cerr << "Trying to access element out of bounds. Lower bound = 0. Upper bound = " 
