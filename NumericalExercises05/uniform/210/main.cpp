...
// Definition of |psi_210|^2
double psi2(double x, double y, double z){
    return z * z * exp(-sqrt(x*x + y*y + z*z) / a0) / (32 * M_PI * pow(a0, 5));
    }
...
int main (int argc, char *argv[]){
    ...
    // Psi 210 with uniform transition rate
    ...
    double w = 6.0 * a0; // maximum width of a transition x1 -> x2
    ...
    FILE *fp1, *fp2, *fp3;
    fp1 = fopen("./data/uniform210.dat", "a");
    if (fp1 == NULL) {
        printf("Can't open uniform210.dat\n");
        exit(EXIT_FAILURE);
    }

    fp2 = fopen("./data/uniform210.xyz", "w");
    if (fp2 == NULL) {
        printf("Can't open uniform210.xyz\n");
        exit(EXIT_FAILURE);
    }
    ...
    for(int i = 0; i < N; i++){
        for(int j = 0; j < L; j++){
            ...
            double y = y0 + w * (rnd.Rannyu() - .5);
            double z = z0 + w * (rnd.Rannyu() - .5);
